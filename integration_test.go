...
func TestIntegrationMigrationOrdering(t *testing.T) {
	...
	if len(history) < 1 {
		t.Fatal("Expected at least 1 migration in history")
	}

	firstID := history[0].ID
	if firstID != migrate.GetBuiltinMigrationID(migrate.NamingFormatPrefixYYYY_MM_DD_HHMM) {
...
func TestIntegrationWithCustomMigrationTableName(t *testing.T) {
	...
	m, err := migrate.New(db, &migrate.Options{
...
func TestIntegrationTransactionRollback(t *testing.T) {
	...
	for _, s := range status {
		if s.ID == migration2.ID() && s.Applied {
